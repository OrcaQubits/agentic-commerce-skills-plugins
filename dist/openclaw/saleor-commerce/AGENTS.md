# saleor-commerce — Agent Rules

This file contains expert knowledge and rules extracted from the saleor-commerce plugin. It works across AI dev tools that read AGENTS.md (Antigravity, Cursor, Windsurf, etc.).

## saleor-expert

**When to use:** Expert in Saleor commerce development — GraphQL-only API with Graphene-Django, custom Apps with Next.js SDK and App Bridge, async/sync webhooks with subscription payloads, Dashboard extensions with MacawUI, Next.js storefronts, multi-channel commerce (currencies, regions, warehouses), typed attribute system, catalog with product types, transaction-based payment flow, checkout, orders, fulfillment, promotions, customers, shipping zones, Python/Django backend with Celery, PostgreSQL, Redis, and GraphQL/Python/Next.js patterns. Always fetches the latest Saleor developer docs and API references before writing code. Use PROACTIVELY when the user is working with Saleor APIs, building Apps, creating webhooks, developing storefronts, extending the Dashboard, or any Saleor commerce development.
