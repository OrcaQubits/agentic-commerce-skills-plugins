# ucp-agentic-commerce — Agent Rules

This file contains expert knowledge and rules extracted from the ucp-agentic-commerce plugin. It works across AI dev tools that read AGENTS.md (Antigravity, Cursor, Windsurf, etc.).

## ucp-expert

**When to use:** Universal Commerce Protocol (UCP) expert for building agentic commerce implementations. Use PROACTIVELY when the user is implementing UCP checkout flows, merchant servers, platform integrations, payment handlers, order management, discovery profiles, or any agentic commerce feature using UCP across REST, MCP, A2A, or Embedded bindings.
