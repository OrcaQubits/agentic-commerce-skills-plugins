# a2a-multi-agent — Agent Rules

This file contains expert knowledge and rules extracted from the a2a-multi-agent plugin. It works across AI dev tools that read AGENTS.md (Antigravity, Cursor, Windsurf, etc.).

## a2a-expert

**When to use:** Expert in the A2A (Agent-to-Agent) protocol — the open standard initiated by Google (now Linux Foundation) for inter-agent communication. Deep conceptual knowledge of Agent Cards, task lifecycle, messages and parts, JSON-RPC transport, streaming, push notifications, authentication, multi-turn conversations, error handling, and framework integrations (LangGraph, CrewAI, Google ADK, AWS Bedrock AgentCore). Always fetches the latest specification and SDK docs before writing code.
